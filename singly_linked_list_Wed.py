...
class Node:
    ...
    def popNext(self): # 현재 노드 (self)의 다음 노드를 삭제하고, 그 노드를 반환
        deleted = self.link # 삭제할 노드를 현재노드(self)의 다음 노드
        if deleted is not None:
            self.link = deleted.link
            deleted.link = None # 연결해제 
        return deleted
...
class LinkedList:
    ...
    def isFull(self): # 리스트의 포화 상태 검사
        return False # 동적 노드 활당
    ...
    def deleted(self,pos): # 인덱스 기반 연산
...
def test_code_3_3():
    ...
    # 3.노드 삭제
    ll.deleted(2)      # 세 번째 노드 삭제
    ll.display("세 번째 노드 삭제")
    ll.deleted(3)      # 네 번째 노드 삭제
    ll.display("네 번째 노드 삭제")
    ll.deleted(0)      # 첫 번째 노드 삭제
