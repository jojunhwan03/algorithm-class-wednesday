...
class CircularDeque(CircularQueueOneSlotEmpty):
    ...
    # self.is_empty() , self.is_full() , self.size() , self.disply() : 상속
    ...
    # 원형에서만 있는 기능 : 자식클래스 구현 
    def add_front(self,item): # 전단에서 삽입
        if self.is_full():
            raise IndexError("원형덱이 포화 -> 삽입불가")
        else:
            self.array[self.front] = item #  먼저 기록
            self.front = (self.front - 1 + self.N) % self.N # front를  반시계 방향으로 회전
            
    def delete_rear(self):#  후단에서 삭제
        if self.is_empty():
            raise IndexError("원형덱이 빈 상태 -> 삭제불가")
        item = self.array[self.rear] # 가장 최근에 저장된 데이터 가리킴
        self.array[self.rear] = None # (option)
        self.rear = (self.rear -1 +self.N )%self.N # rear를 반시계 반향으로 회전
        return item
